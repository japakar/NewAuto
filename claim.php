...
if($_SESSION[$Coin.'claimtime'] < time() - $CConfig['Timer']){ // check if enough time has passed since the last claim
	...
	if(($CConfig['PayoutCycle'] === 1 || $RemainingRefreshes === 0) && !isset($_COOKIE['CreditsEmpty'])) { // check if its time for a withdraw
		...
		$TotalPayout = floor($Payout + $_SESSION[$Coin.'RollOver']); // set the actual payout value (FH doesnt allow Sub-Satoshi values)
		if($TotalPayout === 0){ // check if the actual payout value isnt 0. if so we set the Rollover to the payout + previous rollover
			...
			$Result = ProcessPayout($TotalPayout, $_SESSION[$Coin."address"], $Coin,$CConfig['Apikey'], $Referrer, $Config['RefCommission']); // wrapper function for the FH payout, handles Errors better
			...
			}elseif($Result['status'] === 456 || $Result['status'] === 412){ // the address is invalid: remove the address from the Dataset and End the Session
