...
function GetCurrencies($ApiKey, $Currency){ // wrapper function only used in the admin panel, still making life easier though
	include_once FDSR.DIRECTORY_SEPARATOR."faucethub.php"; // include FH class
	$FH = New FaucetHub($ApiKey,$Currency); // Load the Faucethub library and specify the currency (doesnt matter which we sepcify)
	$response = $FH->getCurrencies(); // do the call vie the library.
...
function ProcessPayout($amount, $to,$currency,$Apikey, $referrer = null, $Commission = 0){ //Convenience Wrapper Function.
	include_once FDSR.DIRECTORY_SEPARATOR."faucethub.php"; // include FH class
	$FH = New FaucetHub($Apikey,$currency); // Load the Faucethub library
	$r = $FH->send($to, $amount, false,$_SERVER['REMOTE_ADDR']); // Send the Payout Request to FH: Result will be processed later.
	if($referrer !== null){ // check if we need to send a referral payout
		...
		if($Reffamount !== 0){ // if its 0 we dont need to send a payout.
			...
			$ReffR = $FH-send($refferer, $Reffamount, true); // send the Referral payout and process it below.
			if(@$ReffR['status'] === 441 || @$ReffR['status'] === 600){ // API limit reached, log this to let the admin know
				...
				if(isset($LimitLog[$Date])){ // increase the limitsReached counter, set the time of LastReached to the current time, and if true, set the Credits empty value to true.
					$LimitLog[$Date] = array("LimitReached" => $LimitLog[$Date]['LimitReached'] + 1, "LastReached" => date("H:i:s"), "CreditsEmpty" => ($LimitLog[$Date]['CreditsEmpty'] ?: ($r['status'] === 600 ?: false)), "CreditsEmptyTime" => ($LimitLog[$Date]['CreditsEmptyTime'] ?: date("H:i:s")));
				}else{
					$LimitLog[$Date] = array("LimitReached" => 1, "LastReached" => date("H:i:s"), "CreditsEmpty" => ($r['status'] === 600 ?: false), "CreditsEmptyTime" => date("H:i:s"));
				}
			...
			}elseif(@$ReffR['status'] === 456 || @$ReffR['status'] === 412){
	...
	if(@$r['status'] === 200){ // process the main payout response
	...
	}elseif(@$r['status'] === 441 || @$r['status'] === 600){ // API limit reached, log this to let the admin know
		$Date = date("d-m"); //get the day and month
		$LimitLog = @file_get_contents(FDSR.DIRECTORY_SEPARATOR."Data".DIRECTORY_SEPARATOR."APIlimit.json");
		$LimitLog = @json_decode($LimitLog, true);
		if($LimitLog === null){ // open the limit log and turn it into a parsable array.
		...
		return array("status" => 911, "success" => false, "message" => "Couldnt connect to Faucethub. your payout will be sent next payout cycle");
	}
}
