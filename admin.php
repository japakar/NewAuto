...
if(!empty($_POST)){ // check if a form was submitted. all changes i made via forms, so this if block is BIG. alot of code all relevant to updating the config
					...
					if(isset($options['Apikey']) && isset($options['Ammount']) && isset($options['Timer']) && isset($options['PayoutCycle'])){
						...
						if((is_int($options['PayoutCycle']) && is_int($options['Timer']) && (is_int($options['Ammount']) || is_float($options['Ammount'])) && CharCheck($options['Apikey']))){
							...
							if($options['Ammount'] !== $CurrInfo['Ammount'] || $options['Timer'] !== $CurrInfo['Timer'] || $options['Apikey'] !== $CurrInfo['Apikey'] || $options['PayoutCycle'] !== $CurrInfo['PayoutCycle']){
								...
								$Config['Currencies'][$currency] = array("Apikey" => $options['Apikey'], "Timer" => $options['Timer'], "Ammount" => $options['Ammount'], "PayoutCycle" => $options['PayoutCycle'], "ConfigID" => $CurrInfo['ConfigID']); // make the change
	...
	UpdateConfig($Config);//update Config
		...
		if(isset($_POST['PayoutCycle']) && isset($_POST['Timer']) && isset($_POST['Ammount']) && isset($_POST['Apikey']) && isset($_POST['currency'])){
			...
			if(is_int($_POST['PayoutCycle']) && is_int($_POST['Timer']) && (is_int($_POST['Ammount']) || is_float($_POST['Ammount'])) && CharCheck($_POST['Apikey'])){
				...
				$res = GetCurrencies($_POST['Apikey'], $_POST['currency']); // use the apikey to check which Currencies FH supports. this is used to validate that the API works
				if ($res['success'] === true) { // check if the call worked.
		            $Currencies = $res['data']['currencies']; // get the array of currencies returned by FH
		            setcookie("FHcurr", json_encode($Currencies), time()+60*60*24); // update the user's Currency Cookie
					if(!in_array($_POST['currency'], $res['data']['currencies'])){ // check if the currency is in the array of returned currencies
						$_SESSION['ErrMSG'] = 'Not a valid currency. please use the dropdown inside the modal for valid currencies.';
						redirect(DIRREC."/admin.php");
					...
					$Config['Currencies'][$_POST['currency']] = array("PayoutCycle" => $_POST['PayoutCycle'], "Timer" => $_POST['Timer'], "Ammount" => $_POST['Ammount'], "Apikey" => $_POST['Apikey'], "ConfigID" => 1); // make the change and notify the admin
					...
					$image = @file_get_contents('https://faucethub.io/assets/img/coins/'.$_POST['currency'].'.png');
					if($image !== false){ // get the image from FHs Server and put it into the images folder so the script can use it
					...
					if($res['data']['status'] === 403){ // the API call returned with an invalid key error, so we notify the admin
						...
						redirect(DIRREC."/admin.php");
					}else{ // some other Error happened, so we let the admin know
						$_SESSION['ErrMSG'] = 'FH Responded with an error: '. $res['data']['message'];
						redirect(DIRREC."/admin.php");
					}
		        }
...
// check if the script has an api key it can use to query FH, otherwhise use a standart set of Currencies
if (!isset($_COOKIE['FHcurr'])) { // check if the user has a set of currencies saved.
	$ValidKey = null; // define $ValidKey
	if(isset($Config['Currencies'])){
		$Currencies = $Config['Currencies']; // Convenience currency array
		shuffle($Currencies); // shuffle the array so that a random key is used if there are multiple active currencies.
		foreach($Currencies as $key => $options){
			$Currency = $key;
			$ValidKey = $options['Apikey'];
		}// iterate over the active currencies, and save the last key and currency

	}else{
		$Currencies = array("BTC","ETH","XMR","LTC","DOGE","BCH","ZEC","DGB","BTX","BLK","DASH","PPC","XPM","POT");
		//if no previous currencies exist we just take a standart array
	}

    if ($ValidKey !== null) { // if the script had atleast one active currency we check if it gave us the apikey.
        $res = GetCurrencies($ValidKey, $Currency); // we call FH's api to check which currencies it supports.

        if ($res['success'] === true) {// if the apicall succeeded we save the data.
            $Currencies = $res['data']['currencies'];
            setcookie("FHcurr", json_encode($Currencies), time()+60*60*24);
        } else { // if the apikey was invalid we let the user know of this ASAP and tell him to check the currency - as this would mean all payouts for that currency would fail
			if($res['data']['status'] === 403){
				$messages = $messages.ErrorMSG("clear", "WARNING: the script detected an invalid Apikey! please change the Apikey for <b>".$res['currency']."!</b> All Apicalls will fail for this key!");
			}
            $Currencies = array("BTC","ETH","XMR","LTC","DOGE","BCH","ZEC","DGB","BTX","BLK","DASH","PPC","XPM","POT"); // the api call wasnt successful so we cast a fallback array of currency we know FH supports
        }
    } else {
        $Currencies = array("BTC","ETH","XMR","LTC","DOGE","BCH","ZEC","DGB","BTX","BLK","DASH","PPC","XPM","POT");
		//there were no active currencies, so we couldnt make the APIcall - use a fallback array instead.
    }
} else { // the user still has an existing dataset.
    $Currencies = json_decode($_COOKIE['FHcurr']);
	if($Currencies === null){ // if the dataset was malformed, we cast a fallback array and remove the dataset so a new one can be fetched on the next reload.
		setcookie("FHcurr", json_encode($Currencies), time()-60*60*24);
		$Currencies = array("BTC","ETH","XMR","LTC","DOGE","BCH","ZEC","DGB","BTX","BLK","DASH","PPC","XPM","POT");
	}
}
...
if(isset($LimitLog[$Date])){ // check if there is an entry for today
	if($Limitlog[$Date]['LimitReached'] > 0){ // check if the script reached any api limits today.
		...
		if(isset($Limitlog[$Date]['CreditsEmpty'])){ // check if the error in question was related to api credits, if so we let the user know
			$messages = $messages.ErrorMSG("clear", "WARNING: the script has reached your API limits! <b>".$Limitlog[$Date]['LimitReached']."</b> APIcalls failed due to this limit today ;/ we reccomend to upgrade your plan to a higher plan or to extend the payout cycle for your currencies<br>NOTICE: you ran out of API credits on FH, this may lead to all further apicalls failing, so we reccomend you to buy more credits as soon as possible.");
											...
											<div class="col-12 col-sm-8">
												<div class="row">
													<div class="col-3 col-sm-3">
														<i class="material-icons " data-toggle="tooltip" data-placement="bottom" title="The ApiKey is Used in order to Communicate with Faucethub. therefore its required to use this currency. you can set each currency to a different APIkey or use the same for all of them, we reccomend to use more than one">help_outline</i>
					...
					<div class="modal fade" id="AddCurrencies">
						<div class="FirstLayer modal-dialog modal-dialog-center modal-lg">
							<div class="modal-content card SecondLayer">
								...
								<p>add a new currency and configure it below! remember that submitting this form wont save any changes outside of it.<br>NOTE: if you havent added a single currency yet you may see all currencies at first. once you add a currency with a valid APIkey you will see all Currencies FH offers</p>
								<div class="row">
									<div class="col-12 col-sm-12">
										<select id="currencySelect" name="currency" class="w-75">
											<?php
												foreach($Currencies as $Currency){ // iterate over all currencies FH supports, skip those active in the script
